...
class Stage2Config:
    ...
    # By default, Stage 2 builds upon the Stage 1 trained projector + chat backbone
    ...
    BATCH_SIZE = 2
    GRAD_ACCUM_STEPS = 4
    LR = 2e-5           # End-to-end multimodal fine-tuning
    MIN_LR = 1e-6
    WEIGHT_DECAY = 0.01
    GRAD_CLIP = 1.0
    EPOCHS = 8
...
class Stage2InstructionConfigurator(ITrainingStageConfigurator):
    def configure(self, bridge: VisionBridge, slm: nn.Module, lr: float) -> Optimizer:
        ...
        return AdamW(trainable_params, weight_decay=Stage2Config.WEIGHT_DECAY)
...
def train_stage2(data_path: Optional[str] = None, base_checkpoint: Optional[str] = None, epochs: int = Stage2Config.EPOCHS, lr: Optional[float] = None):
    ...
    mlp_projector = MLPProjector(visual_dim=vision_encoder.hidden_dim, language_dim=Stage2Config.NINP).to(device)
    bridge = VisionBridge(encoder=vision_encoder, projector=mlp_projector).to(device)

    # 2. Initialize Language Model Backbone
    model = TransformerModel(
        ntoken=vocab_size,
        ninp=Stage2Config.NINP,
        nhead=Stage2Config.NHEAD,
        n_kv_heads=Stage2Config.N_KV_HEADS,
        nhid=Stage2Config.NHID,
        nlayers=Stage2Config.NLAYERS,
        dropout=Stage2Config.DROPOUT,
        max_seq_len=Stage2Config.MAX_SEQ_LEN
    ).to(device)

    # 3. Load pre-trained weights
    # Priority for Stage 2:
    # A. Stage 1 Projector checkpoint (has both projector weights and model weights)
    # B. Base chat checkpoint (nandi_chat_final.pt)
    # C. Explicit override via base_checkpoint
    ckpt_to_load = base_checkpoint or Stage2Config.STAGE1_PROJECTOR_CHECKPOINT
    if not os.path.exists(ckpt_to_load):
        ckpt_to_load = Stage2Config.BASE_CHAT_CHECKPOINT

    if not os.path.exists(ckpt_to_load):
        fallback_ckpts = sorted([
            os.path.join(Stage2Config.DEFAULT_CHECKPOINT_DIR, f)
            for f in os.listdir(Stage2Config.DEFAULT_CHECKPOINT_DIR)
            if f.endswith(".pt")
        ]) if os.path.exists(Stage2Config.DEFAULT_CHECKPOINT_DIR) else []
        if fallback_ckpts:
    ...
    learning_rate = lr or Stage2Config.LR
    ...
    resolved_data_path = data_path or Stage2Config.DEFAULT_DATA_PATH
    ...
    dataloader = DataLoader(dataset, batch_size=Stage2Config.BATCH_SIZE, shuffle=True, drop_last=False)
    total_opt_steps = (len(dataloader) // Stage2Config.GRAD_ACCUM_STEPS) * epochs
    scheduler = CosineAnnealingLR(optimizer, T_max=max(total_opt_steps, 1), eta_min=Stage2Config.MIN_LR)

    print(f"[+] Dataset size: {len(dataset)} samples | Batch Size: {Stage2Config.BATCH_SIZE} | Epochs: {epochs}")
    print(f"[+] Effective Batch Size: {Stage2Config.BATCH_SIZE * Stage2Config.GRAD_ACCUM_STEPS}")

    # 6. Training Loop
    os.makedirs(Stage2Config.DEFAULT_CHECKPOINT_DIR, exist_ok=True)
    global_step = 0
        ...
        for step, (pixel_values, input_ids, labels) in enumerate(dataloader):
            ...
            if use_amp:
                with torch.autocast(device_type=device.type, dtype=amp_dtype):
                    ...
                    loss = loss / Stage2Config.GRAD_ACCUM_STEPS
            else:
                loss = trainer.execute_step(pixel_values, input_ids, labels)
                loss = loss / Stage2Config.GRAD_ACCUM_STEPS
            ...
            if (step + 1) % Stage2Config.GRAD_ACCUM_STEPS == 0 or (step + 1) == len(dataloader):
                torch.nn.utils.clip_grad_norm_(
                    list(bridge.projector.parameters()) + list(model.parameters()),
                    max_norm=Stage2Config.GRAD_CLIP
                )
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad(set_to_none=True)
                ...
                if global_step % Stage2Config.EVAL_INTERVAL == 0 or global_step == 1:
                    curr_loss = loss.item() * Stage2Config.GRAD_ACCUM_STEPS
            ...
            epoch_loss += loss.item() * Stage2Config.GRAD_ACCUM_STEPS

        avg_loss = epoch_loss / len(dataloader)
        print(f"Epoch {epoch + 1} Complete | Average Loss: {avg_loss:.4f}")

    # 7. Save Final Multimodal Checkpoint
    final_out = Stage2Config.OUTPUT_FINAL_CHECKPOINT
    torch.save({
        "stage": 2,
        "step": global_step,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=Stage2Config.DEFAULT_DATA_PATH, help="Path to multimodal training JSONL dataset.")
    ...
    parser.add_argument("--epochs", type=int, default=Stage2Config.EPOCHS, help="Number of training epochs.")
