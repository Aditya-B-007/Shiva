...
# =============================================================================
# Paths (stage-specific; referenced from config for numeric hyperparams)
# =============================================================================
...
# =============================================================================
# Stage 1 configurator — freezes SLM + encoder, trains projector only
# =============================================================================
...
class Stage1AlignmentConfigurator(ITrainingStageConfigurator):
    def configure(self, bridge: VisionBridge, slm: nn.Module, lr: float) -> Optimizer:
        ...
        return AdamW(
            ...
            lr=lr,
            weight_decay=default_stage1_config.weightDecay
        )
...
def train_stage1(
    ...
    # 3. Load pre-trained language weights
    ckpt_to_load = base_checkpoint or BASE_CHAT_CHECKPOINT
    if not os.path.exists(ckpt_to_load):
        ...
        if fallback_ckpts:
            ...
            print(f"[!] Target chat checkpoint not found. Falling back to latest: {ckpt_to_load}")
        else:
    ...
    for epoch in range(epochs):
        ...
        for step, (pixel_values, input_ids, labels) in enumerate(dataloader):
            ...
            if (step + 1) % cfg.gradAccumSteps == 0 or (step + 1) == len(dataloader):
                torch.nn.utils.clip_grad_norm_(bridge.projector.parameters(), max_norm=cfg.gradClip)
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad(set_to_none=True)
                global_step += 1
    ...
    torch.save({
        ...
        "loss": avg_loss,
    }, OUTPUT_STAGE1_CHECKPOINT)
    print(f"\n[+] Stage 1 Alignment Complete! Checkpoint saved to: {OUTPUT_STAGE1_CHECKPOINT}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_checkpoint", type=str,   default=None,                help="Path to base language checkpoint.")
    parser.add_argument("--epochs",          type=int,   default=default_stage1_config.epochs, help="Number of training epochs.")
