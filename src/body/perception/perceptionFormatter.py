...
from src.body.perception.perceptionDTOs import (
    ObservationKind,
    PerceptionBundleDTO,
...
class PerceptionObservationFactory:
    ...
    def _classify(self, device: str, payload: Any) -> ObservationKind:
        ...
        if isinstance(payload, bytes):
            if "camera" in device_name or "screen" in device_name:
                return ObservationKind.IMAGE_BYTES
            if "microphone" in device_name or "audio" in device_name:
                return ObservationKind.AUDIO_BYTES
    ...
    def _summarize(
        ...
        if kind in {
            ObservationKind.IMAGE_BYTES,
            ObservationKind.AUDIO_BYTES,
            ObservationKind.BINARY_BYTES,
        }:
            return f"{device} captured {kind.value} payload ({payload_size} bytes)."
