...
class RotaryPositionalEncoding(PositionalEncodingInterface):
    def __init__(self, config):
        ...
        self._build_cache(self.max_sequence_length)

    def _build_cache(self, sequence_length):
        device = self.cos_cache.device if hasattr(self, "cos_cache") else torch.device("cpu")
        inv_frequency = 1.0 / (self.theta ** (torch.arange(0, self.head_dimension, 2, device=device) / self.head_dimension))
        positions = torch.arange(sequence_length, device=device)
        frequencies = torch.outer(positions.float(), inv_frequency.float())
        frequencies = torch.cat((frequencies, frequencies), dim=-1)

        self.register_buffer("cos_cache", frequencies.cos().unsqueeze(0).unsqueeze(0), persistent=False)
        self.register_buffer("sin_cache", frequencies.sin().unsqueeze(0).unsqueeze(0), persistent=False)
