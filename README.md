...
flowchart TD
    ...
    ES["EnvironmentStack<br/>repr C align 64"]
    ...
    subgraph P2["Phase 2: Unconstrained Candidate Consensus"]
        FD["Fast Decision Engine<br/>SAC<br/>a_fast, w_fast"]

        LV["Long Vision Engine<br/>IQN<br/>w_risk"]

        EX["Explorer Engine<br/>TD3 + z<br/>a_explore, w_adapt"]

        MERGE["Normalized Weighted Reduction<br/>Candidate Action"]
    end

    subgraph P3["Phase 3: Immutable Safety Shield"]
        GR["GuardRail Engine<br/>CPO Post-Pass Projection<br/>Slew Rate Limiting<br/>Rule Mask Interlocks<br/>Convex Set Projection"]
    end

    EMG["Emergency Recovery Action<br/>a_emergency"]

    PREV["Hold Previous Action<br/>Safety Fallback"]

    HW["Final Hardware Dispatch<br/>Safe Command a_t"]
